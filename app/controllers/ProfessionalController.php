...
class ProfessionalController extends \BaseController {
	...
	public function add(){	
		...
		if($exam != 'Civil Examination'){
		Letscorer::create(array(
			'name'		=>$name,
			'score'		=>$score,
			'exam'		=>$exam,
			'school'	=>$school
	...
	public function ShowError(){	
		return View::make('professional-education-error');		
	}
	public function RecordProfessional(){
		
		$name= Config::get('professionalgenerator.some_key');	
		...
		if (is_null($reg)){
		$adduser=DB::insert('insert into users (user, item, exam) values (?, ?, ?)', array($name, '1', 'Professional Education'));    
		} else {
		$update =DB::table('users')
						->where('user', $name)
						->update(array('updated_at'	=> $date, 'exam'=> 'Professional Education'));		
		}	
		...
		$db->statement($sql);
		$sql = "INSERT IGNORE INTO `".$name."` SELECT * FROM proexams ON DUPLICATE KEY UPDATE `up` = 'done'";
		$db->statement($sql);
		
		return View::make('professional-education');
		
	}		
	
	public function ProfessionalCheck(){	
		$date = date('Y-m-d H:i:s');
		$name= Config::get('professionalgenerator.some_key');
		$item = Session::get('id');
		...
		if($ans==$answer){				
			...
			return Redirect::to('professional-education-error');					
		}
	
	}	
	
	public function ProfessionalSuggest(){			
		$ii = Session::get('id');
		$date = date('Y-m-d H:i:s');
		...
		$rr = DB::table('professionalerrors')->where('item', '=', $ii)->first(); 		
		
		if (is_null($rr)){
		$add=DB::insert("insert into professionalerrors (item, $answer) values (?, ?)", array($ii, 1));    
		}else{
		$score=($rr->$answer);
		$update =DB::table('professionalerrors')->where('item', $ii)->update(array($answer =>  $score + 1, 'updated_at' => $date));		
		}		
		//get the data
		$datainfo = DB::table('professionalerrors')
				->Where('item', $ii)	
				->get();
